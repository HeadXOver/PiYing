...
piying::tool::part::TriangleRectSelect::TriangleRectSelect() :
...
void piying::tool::part::TriangleRectSelect::draw()
{
...
void piying::tool::part::TriangleRectSelect::click(const QPointF& mouse)
{
	_is_press = true;
	chTriangleSelect->mouse_press(mouse);
}

void piying::tool::part::TriangleRectSelect::mouse_move(const QPointF& mouse)
{
	if (!_is_press) return;
...
void piying::tool::part::TriangleRectSelect::release(const QPointF& mouse)
{
	_is_press = false;
	...
	for (unsigned int i = 0; i < triangleIndices.size(); i += 3) {
		for (int j = 0; j < 3; ++j) eachTriangle[j] = PiYingGL::getInstance().mapViewProjMatrix(pointVector.get(triangleIndices[i + j], true));
		...
		if (rect.contains(eachTriangle[0]) &&
			...
			if (!containTriangle) chTriangleSelect->selected_trangle->append(triangle);
		}
	}
...
void piying::tool::part::TriangleRectSelect::delete_element()
{
	chTriangleSelect->deleteElement();
}

void piying::tool::part::TriangleRectSelect::escape()
{
