...
void PiYingGL::draw_selected_points(size_t nSelectedPoint)
{
	...
	glDrawArrays(GL_POINTS, 0, static_cast<int>(nSelectedPoint));
...
void PiYingGL::draw_selected_triangle(size_t nSelectedPoint)
{
	...
	_texture_tri_shader_program->setUniformValue("is_skelen", editMode != EditMode::characterTexture);
	_texture_tri_shader_program->setUniformValue("aColor", QVector4D(0.0f, 0.0f, 1.0f, 0.15f));
	glDrawElements(GL_TRIANGLES, static_cast<int>(nSelectedPoint) * 3, GL_UNSIGNED_INT, 0);
...
void PiYingGL::paintBackgrounds()
{
	...
	for (int i = 0; i < backGrounds.size(); i++) {
		piying::ImageTexture* it = backGrounds[i];
		it->bind();
...
void PiYingGL::update_view_matrix_of_shaders()
{
	...
	makeCurrent();
	_ch_shader_program->bind();
	_ch_shader_program->setUniformValue("view_proj", getViewProjMatrix());
	_selected_vert_shader_program->bind();
...
void PiYingGL::update_selected_triangle(const unsigned int* data, size_t size)
{
	makeCurrent();
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ttEBO);
...
void PiYingGL::update_selected_triangle_texture()
{
	int currentVector = getCurrentChRow();
	if (currentVector < 0) return;
...
void PiYingGL::update_selected_triangle_part()
{
	const PointVectorLayer* layer = TimelineGl::getInstance().currentLayerOrigin();
	if (!layer) return;

	makeCurrent();
	glBindBuffer(GL_ARRAY_BUFFER, ttVBO);
	glBufferData(GL_ARRAY_BUFFER, layer->float_size() * sizeof(float), layer->data(), GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	doneCurrent();
}
