...
void piying::tool::part::TriangleSelect::escape()
{
    if (selected_trangle->isEmpty()) return;
    selected_trangle->remove_last_triangle();
}

void piying::tool::part::TriangleSelect::deleteElement()
{
...
void piying::tool::part::TriangleSelect::draw_handle_and_selected()
{
    if (selected_trangle->isEmpty()) return;

    // 计算中心点
    PointVectorLayer& pointLayer = *TimelineGl::getInstance().currentLayerOrigin();
    _gl_handle_center = QPointF();
    for (unsigned int i : selected_trangle->index()) {
        _gl_handle_center += pointLayer[i];
    }
...
void piying::tool::part::TriangleSelect::mouse_press(const QPointF& mouse)
{
    lastPos = mouse;
...
void piying::tool::part::TriangleSelect::moveHandle(const QPointF& mouse)
{
    ...
    case HandleControlMode::Move: {
        ...
        for (int i = 0; i < selected_trangle->point_size(); i++) {
            toSet = selected_trangle->getVert(i) + toMove;
    ...
    case HandleControlMode::Rotate: {
        ...
        for (int i = 0; i < selected_trangle->point_size(); i++) {
            toSet = rotation.map(selected_trangle->getVert(i)) + mappedLastHandleCenterPoint;
            pointLayerOrigin.set_point(true, (*selected_trangle)[i], toSet);
            pointLayer.set_point(true, (*selected_trangle)[i], toSet);
        }
    }break;
    case HandleControlMode::Scale: {
        ...
        QPointF toSet;
        for (int i = 0; i < selected_trangle->point_size(); i++) {
            toSet = selected_trangle->getVert(i) * scale - toScale;
            pointLayer.set_point(true, (*selected_trangle)[i], toSet);
            pointLayerOrigin.set_point(true, (*selected_trangle)[i], toSet);
        }
    }break;
    case HandleControlMode::ScaleX: {
        ...
        QPointF toSet;
        for (int i = 0; i < selected_trangle->point_size(); i++) {
            toSet = trans.map(selected_trangle->getVert(i));
            pointLayer.set_point(true, (*selected_trangle)[i], toSet);
            pointLayerOrigin.set_point(true, (*selected_trangle)[i], toSet);
        }
    }break;
...
void piying::tool::part::TriangleSelect::affirmHandle()
{
    selected_trangle->affirmVert(*TimelineGl::getInstance().currentLayerOrigin(), true);
    lastHandleCenterPoint = _gl_handle_center;
    lastDHandleCenterPoint = _widget_handle_center;
}

void piying::tool::part::TriangleSelect::click_select(const QPointF& mouse)
{
...
void piying::tool::part::TriangleSelect::update_selected_to_draw()
{
    PiYingGL::getInstance().update_selected_triangle(selected_trangle->index_data(), selected_trangle->point_size());
    PiYingGL::getInstance().update_selected_triangle_part();
}
