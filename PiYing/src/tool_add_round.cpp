...
void piying::tool::AddRound::click(const QPointF& mouse)
{
	center = mouse;
...
void piying::tool::AddRound::mouse_move(const QPointF& mouse)
{
	radius = QLineF(center, mouse).length();
	current_cursor = mouse;
}

void piying::tool::AddRound::release(const QPointF& mouse)
{
	if (radius < 6) {
		isPress = false;
		return;
	...
	int lenth = QLineF(QPointF(), get_proj_invert().map(glCursor - gl_center)).length() * 1000.f;

	int init[3] { lenth, 7, init_angle };
	int outEdgeCount;
...
void piying::tool::AddRound::draw()
{
	if(!isPress) return;
...
void piying::tool::AddRound::addRoundPoly(const int edgeCount)
{
	if (edgeCount < 3) return;

	const float lenth = radius / 1000.f;
	const double initAngle = (init_angle - PiYingGL::getInstance().view_rotate_degree()) * angle_rad;
