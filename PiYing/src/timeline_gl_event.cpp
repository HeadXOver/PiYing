...
void TimelineGl::wheelEvent(QWheelEvent* ev)
{
	if (_ui_type == spTimelineGL::UiType::Timeline) {
		...
		_scale_trans->trans_x = piying::min(_scale_trans->scale_lenth - 1.f, _scale_trans->trans_x + to_trans_x);
...
void TimelineGl::mouseMoveEvent(QMouseEvent* event)
{
	if (event->buttons() == Qt::LeftButton) {
		...
		_scale_trans->trans_x = piying::min(_scale_trans->scale_lenth - 1.f, to_trans_x);
		...
		if (timeline_size > 5) {
			...
			to_trans_y = piying::max(_scale_trans->scale_height - 1.f, to_trans_y);
			_scale_trans->trans_y = piying::min(0.4f * timeline_size - 2.f, to_trans_y);
		}
		update();
	}
}
