...
void TimelineGl::update_current_part_scale()
{
	if (_showing_parts.size() == 0) return;

	_showing_parts[_part_cursor._index]->update_scale();
}

PointVectorLayer* TimelineGl::currentLayer() const
{
	if (_showing_parts.size() == 0) return nullptr;

	return &_showing_parts[_part_cursor._index]->vert_layer();
}

PointVectorLayer* TimelineGl::currentLayerOrigin() const
{
	if (_showing_parts.size() == 0) return nullptr;

	return &_showing_parts[_part_cursor._index]->vert_layer_origin();
}
...
void TimelineGl::move_time_cursor(float mouse_x)
{
	...
	time_cursor = piying::max(half_width * (1.f - _scale_trans->scale_lenth), mouse_x - _scale_trans->trans_x * half_width); ///< 确保非负
}
...
std::vector<unsigned int>* TimelineGl::get_current_indices()
{
	if (_showing_parts.size() == 0) return nullptr;

	if (_part_cursor._index < 0) return nullptr;

	return &_showing_parts[_part_cursor._index]->indices();
}

void TimelineGl::insert_from_to(int from, int to)
{
	assert(from >= 0 && to >= 0);
	assert(from < Parts::getInstance().size() && to <= Parts::getInstance().size());
