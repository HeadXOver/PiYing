	...
	for (unsigned int i = 0; i < indices.size(); ++i) {
		...
		if (eachIndex < 0) {
			...
			if (isTexture) {
				...
				left = piying::min(left, eachPoint.x());
				right = piying::max(right, eachPoint.x());
			}
			else {
				eachPoint = currentLayer[indices[i]];
...
bool Part::is_point_traced(unsigned int index) const noexcept
{
	return _slide_applier->contains(index);
}
...
float Part::local_top() const
{
	...
	for (int i = 1; i < _vert_texture->element_size(); ++i) {
		top = piying::max(top, _vert_texture->get(i, true).y());
	}
...
float Part::local_bottom() const
{
	...
	for (unsigned int i = 1; i < _vert_texture->element_size(); ++i) {
		bottom = piying::min(bottom, _vert_texture->get(i, true).y());
	}
...
float Part::local_left() const
{
	...
	for (unsigned int i = 1; i < _vert_texture->element_size(); ++i) {
		left = piying::min(left, _vert_texture->get(i, true).x());
	}
...
float Part::local_right() const
{
	...
	for (unsigned int i = 1; i < _vert_texture->element_size(); ++i) {
		right = piying::max(right, _vert_texture->get(i, true).x());
	}
...
float Part::global_top() const
{
	...
	for (Part* child : _children) {
		top = piying::max(top, child->global_top());
	}
...
float Part::global_bottom() const
{
	...
	for (Part* child : _children) {
		bottom = piying::min(bottom, child->global_bottom());
	}
...
float Part::global_left() const
{
	...
	for (Part* child : _children) {
		left = piying::min(left, child->global_left());
	}
...
float Part::global_right() const
{
	...
	for (Part* child : _children) {
		right = piying::max(right, child->global_right());
	}
