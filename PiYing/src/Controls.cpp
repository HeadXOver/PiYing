...
void PiYingGL::bgRotationControl(const QPointF& mouse, piying::ImageTexture& image)
{
	...
	image.add_rot_radian(piying::angleBetweenPoint(vec2, vec1));
}

void PiYingGL::bgTranslateControl(const QPointF& mouse, piying::ImageTexture& image)
{
	setCursor(Qt::ClosedHandCursor);
	image.set_transform(*lastImageTransform);
	image.addTrans(piying::getRotatedPoint(_orth_ratio_invert.map(mouse - lastMousePos) / viewScale.value(), -viewRotate.value() * angle_rad));
}

void PiYingGL::bgScaleControl(const QPointF& mouse, piying::ImageTexture& image)
{
