...
public:
	...
	void update_vbo(const PointVectorLayer& pointVector, unsigned int vbo);
	...
	void update_current_part_scale();
	void delete_part_all_vert();
	void part_delete();

	PointVectorLayer* currentLayer() const;
	PointVectorLayer* currentLayerOrigin() const;

	std::vector<unsigned int>* get_current_indices();
