...
{
public:
	Part(piying::Texture& texture, const QList<unsigned int>& indices, bool isTexture);
...
public:
	...
	bool is_point_traced(unsigned int index) const noexcept;

	Part* get_child(size_t index) const;
	Part* get_parent() const noexcept { return _parent; }

	PointVectorLayer& vert_layer() noexcept { return *_vert_texture; }
	PointVectorLayer& vert_layer_origin() noexcept { return *_vert_texture_origin; }

	std::vector<unsigned int>& indices() noexcept { return _indices; }
